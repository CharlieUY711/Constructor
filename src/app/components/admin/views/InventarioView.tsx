      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => { setForm({}); setEditing(null); setShowForm(true); }}
            style={{ background: ORANGE, color: '#fff', border: 'none', borderRadius: 8, padding: '9px 16px', fontWeight: 600, cursor: 'pointer' }}>
            + Nuevo
          </button>
        </div>
          ...
          : (
            <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {items.length === 0
                    ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: '#9CA3AF' }}>No hay items</td></tr>
                    : items.map((item: any) => {
                        const bajo = item.cantidad <= item.cantidad_minima;
                        return (
                          <tr key={item.id} style={{ borderBottom: '1px solid #F3F4F6', background: bajo ? '#FFFBEB' : '#fff' }}>
                            <td style={{ padding: '12px 16px', fontFamily: 'monospace', fontWeight: 600 }}>{item.sku}</td>
                            <td style={{ padding: '12px 16px' }}>
                              {bajo && <AlertTriangle size={13} style={{ color: '#F59E0B', marginRight: 4, verticalAlign: 'middle' }} />}
                              {item.nombre}
                            </td>
                            <td style={{ padding: '12px 16px', color: '#6B7280', fontSize: 13 }}>{item.depositos?.nombre || '-'}</td>
                            <td style={{ padding: '12px 16px', fontWeight: 700, color: bajo ? '#D97706' : '#111' }}>{item.cantidad}</td>
                            <td style={{ padding: '12px 16px', color: '#9CA3AF', fontSize: 13 }}>{item.cantidad_minima}</td>
                            <td style={{ padding: '12px 16px' }}>
                              <button onClick={() => setShowMov(item)}
                                style={{ background: '#F0FDF4', border: 'none', borderRadius: 6, padding: '5px 8px', cursor: 'pointer', color: '#059669', marginRight: 4 }}>
                                <ArrowUp size={13} />
                              </button>
                              <button onClick={() => { setForm(item); setEditing(item.id); setShowForm(true); }}
                                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6B7280', marginRight: 4 }}><Edit2 size={13} /></button>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: '#fff', borderRadius: 14, padding: 28, width: '100%', maxWidth: 400 }}>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button onClick={registrar} disabled={saving} style={{ padding: '9px 18px', background: ORANGE, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, cursor: 'pointer' }}>
