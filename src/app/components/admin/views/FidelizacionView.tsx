    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', backgroundColor: '#F8F9FA' }}>
        <div style={{ padding: '24px 28px', maxWidth: '1200px' }}>
          ...
          {activeTab === 'overview' && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
                ...
                {[
                  { label: 'Puntos canjeados',  value: totalPuntosCanjeados.toLocaleString(), Icon: Gift,        color: '#F59E0B' },
                  { label: 'Puntos activos', value: totalPuntosActivos.toLocaleString(),  Icon: DollarSign,  color: '#10B981' },
                  { label: 'Niveles configurados', value: niveles.length.toString(),   Icon: TrendingUp,  color: '#7C3AED' },
                ].map((s, i) => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
                ...
                <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px' }}>
                  ...
                  {levelDistribution.length > 0 ? (
                    ...
                    <p style={{ textAlign: 'center', color: '#9CA3AF', fontSize: '0.8rem' }}>No hay datos</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                {TIERS.length > 0 ? (
                  ...
                  <p style={{ textAlign: 'center', color: '#9CA3AF', fontSize: '0.8rem' }}>No hay niveles configurados</p>
                )}
              </div>
                </>
              )}
          ...
          {activeTab === 'members' && (
            <>
              {loading ? (
                ...
                <>
              {selectedMiembro ? (
                <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: '800', color: '#111827' }}>
                      {selectedMiembro.nombre || selectedMiembro.email}
                    </h3>
                    <button onClick={() => setSelectedMiembro(null)} style={{ padding: '8px 16px', border: '1px solid #E5E7EB', borderRadius: '8px', backgroundColor: '#FFF', color: '#374151', fontSize: '0.82rem', cursor: 'pointer' }}>
                      ← Volver
                    </button>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                    <div>
                      <p style={{ margin: '0 0 4px', fontSize: '0.78rem', color: '#6B7280' }}>Email</p>
                      <p style={{ margin: 0, fontWeight: '600', color: '#111827' }}>{selectedMiembro.email}</p>
                    </div>
                    <div>
                      <p style={{ margin: '0 0 4px', fontSize: '0.78rem', color: '#6B7280' }}>Puntos actuales</p>
                      <p style={{ margin: 0, fontWeight: '800', color: ORANGE, fontSize: '1.2rem' }}>{selectedMiembro.puntos_actuales.toLocaleString()}</p>
                    </div>
                  </div>
                  <button onClick={() => handleAddPuntos(selectedMiembro.id)} style={{ padding: '10px 20px', backgroundColor: ORANGE, color: '#FFF', border: 'none', borderRadius: '8px', fontWeight: '700', cursor: 'pointer', fontSize: '0.875rem', marginBottom: '20px' }}>
                    <Plus size={14} style={{ display: 'inline', marginRight: '6px' }} /> Ajustar Puntos
                  </button>
                  <h4 style={{ margin: '0 0 12px', fontSize: '0.9rem', fontWeight: '700', color: '#111827' }}>Historial de Movimientos</h4>
                    ...
                    {selectedMiembro.movimientos && selectedMiembro.movimientos.length > 0 ? (
                      selectedMiembro.movimientos.map(mov => (
                        <div key={mov.id} style={{ padding: '12px', borderBottom: '1px solid #F3F4F6', display: 'flex', justifyContent: 'space-between' }}>
                          <div>
                            <p style={{ margin: 0, fontWeight: '600', color: '#111827', fontSize: '0.875rem' }}>{mov.descripcion || 'Movimiento'}</p>
                            <p style={{ margin: '2px 0 0', fontSize: '0.72rem', color: '#6B7280' }}>{new Date(mov.created_at).toLocaleString('es-ES')}</p>
                          </div>
                          <span style={{ fontWeight: '700', color: mov.tipo === 'suma' ? '#16A34A' : '#EF4444', fontSize: '0.875rem' }}>
                            {mov.tipo === 'suma' ? '+' : '-'}{mov.puntos}
                          </span>
                        </div>
                      ))
                    ) : (
                      <p style={{ textAlign: 'center', color: '#9CA3AF', fontSize: '0.8rem', padding: '20px' }}>No hay movimientos</p>
                    )}
                  </div>
                </div>
              ) : (
                <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '12px', overflow: 'hidden' }}>
                  <div style={{ padding: '16px 20px', borderBottom: '1px solid #E5E7EB', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '0.8rem', color: '#6B7280' }}>{miembros.length} miembros</span>
                  </div>
                  {miembros.length === 0 ? (
                            ...
                            <tr key={m.id} style={{ borderTop: '1px solid #F3F4F6', backgroundColor: i % 2 === 0 ? '#FFF' : '#FAFAFA' }}>
                              <td style={{ padding: '12px 16px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                  <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: `linear-gradient(135deg, ${nivelColor}, ${nivelColor}88)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFF', fontWeight: '700', fontSize: '0.8rem' }}>
                                    {(m.nombre || m.email).charAt(0).toUpperCase()}
                                  </div>
                                  <span style={{ fontWeight: '600', color: '#111827', fontSize: '0.875rem' }}>{m.nombre || m.email}</span>
                                </div>
                              </td>
                              ...
                              <td style={{ padding: '12px 16px' }}>
                                <button onClick={() => handleViewMiembro(m.id)} style={{ padding: '6px 12px', backgroundColor: ORANGE, color: '#FFF', border: 'none', borderRadius: '6px', fontSize: '0.75rem', fontWeight: '600', cursor: 'pointer' }}>
                                  Ver
                                </button>
                              </td>
                            </tr>
          ...
          {activeTab === 'rewards' && (
            <div>
              <p style={{ textAlign: 'center', color: '#9CA3AF', fontSize: '0.8rem', padding: '40px' }}>
